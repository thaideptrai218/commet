...
| `VerificationCoordinator` | `<<coordinator>>` | UC-13 Submit Owner Verification |
| `RequestReviewCoordinator` | `<<coordinator>>` | UC-14 Review Rental Request, UC-15 Reopen Room Listing |
| `AdminCoordinator` | `<<coordinator>>` | UC-16 Review Verification, UC-17 Manage Account, UC-18 Control Listing |

> All coordinators are stateless `<<coordinator>>`. State machines live in entities (RoomListing.status, RentalRequest.status, OwnerVerification.status, User.accountStatus). State-dependent control evaluation deferred to Step 2.3.
...
| `RoomListingLogic` | `<<business logic>>` | Publication gate (owner must be verified); status/visibility transitions (Draft → Published → Locked / Hidden / Archived) |
...
| `VerificationLogic` | `<<business logic>>` | Approve/reject verification decisions; owner publishing eligibility |
...
| UC-16 Review Owner Verification | A | AdminUI → AdminCoordinator → VerificationLogic → OwnerVerification (+EmailProxy via NotificationService) |
